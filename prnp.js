const form = document.getElementById("prnp-form");
...
form.addEventListener("submit", function (event) {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      <td>${'P'+i}</td>
       ...
       <td> <label for="burst-time-${i}"> <input type="number" class= "inout_style"  id="priority-${i}" name="priority-${i}" min="1" required></label></td>
    </tr>
      `;
    }
...
document.getElementById("calculate-btn").addEventListener("click", function (event) {
  event.preventDefault();
    ...
    for (let i = 1; i <= n; i++) {
      ...
      proces.push(new proc(i,arrivalTime,burstTime,priority));
    }
    if(d != 1){
      proces.sort((m, n) => {
